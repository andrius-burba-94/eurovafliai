...
| **choose** | Tapping a player to say you mean them, without drafting them. The row reads `Choose`, and once tapped it reads **chosen** and is struck in the clock's marker. It is the first of two taps — the second is *draft*. Called *arming* in code (`armed-pick.tsx`), a word deliberately kept out of the interface: 3.7's critique found four labels for one act and this table defining none of them. | `ArmedPickProvider`, `ChooseButton` |
| **draft** (verb) | Committing the chosen player. The one act on the surface, and the only marker-red control in the room: `Draft` in the on-the-clock band. Distinct from *pick*, which is the **noun** for what lands on the board — you *draft* a player and the result *is* a pick. | `makePick`, `ConfirmPick` |
