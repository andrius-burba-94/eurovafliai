...
| **Autodraft has nothing to rank on** | `selectAutoPick` ranks a cheat sheet first and projections second, and neither exists yet: cheat sheets are 3.4, projections 4.4. So today it takes the **first legal player alphabetically** — deterministic, replayable and honest about being arbitrary, which is why `readPool` sorts by name rather than leaving the order to whatever the database returned. It is also not a placeholder that can rot quietly: the pool is passed `projectedPoints` the moment the field exists | Nothing; autodraft quality, not correctness |
...
| `npm run test` | **355 passed** — 173 the engine, 55 the ingestion pipeline, 55 leagues and the draft setup, 37 the clock and the sweep's own decisions, 35 config and helpers |
| `npm run build` | pass |
...
the awkward states (a lost deadline, a hole in the board, a race lost to a human
on zero) against a strict PocketBase fake that enforces the real unique indexes
and refuses any filter it cannot parse, and `tests/e2e/worker.spec.ts` runs the
same `sweepOnce` against the real database — scoped with `onlyDraft`, so a suite
run cannot autodraft into a league somebody was testing by hand.
