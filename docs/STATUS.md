...
| **A rollback never says how many picks it will discard** | The one item worth taking out of the cut 3.6 (blueprint D13), and the only part of it that is not comfort. The form takes a pick number and says "That pick and everything after it is discarded. 20 picks have been made" — it does **not** say what *this* number would cost. Type `1` for `11` with twenty picks on the board and twenty are gone, irreversibly: a rollback deletes picks, and there is no un-rollback. The engine already computes `deletePickIds.length`, so naming it is a read rather than new logic. It is also the exact pattern three consecutive critiques have landed on — name the number before something irreversible (3.4a's delete confirmation named the *wrong* number; 3.4b's remove and 3.5's delete both ended up with an undo) | Nothing; the most destructive control in the app is the one that names no number |
