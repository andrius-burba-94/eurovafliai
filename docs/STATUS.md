...
*display* a box score at all. Phase 3 is closed apart from one thing no code can
finish — the **human rehearsal** its DoD asks for, a draft night with 3+ friends
on mixed devices, inherited from Phase 2 (blueprint D12). That is the only claim
in this file no test can make. 3.3 stays `partial` until **4.4**, because its
one deferred filter needs projections to filter on.

## Try it on localhost — the code-health pass

```bash
npm run lint:dead        # knip: unused files, exports, dependencies — now a CI job
npm run test             # 882 unit tests; the stores and repairs are covered now
...
| `npm run test` | **882 passed.** The engine, the sweep and the pipeline, ingestion, leagues and draft setup, components, cheat sheets (parse, match, round-trip, edit arithmetic), the pool, the design tokens, the on-the-clock cue, league chat — plus, new in this pass, the framework-free stores for sheets, chat, stats and rosters, and both repairs (`reconcileLeagueStatus`, `ensureCommissionerMembership`), each over `fake-pb` with its unique indexes enforced |
| `npm run build` | pass |
...
| `npm run pb:verify` | **105 checks pass** |
