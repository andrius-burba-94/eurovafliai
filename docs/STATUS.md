...
| **The depth scale is prose, and nothing enforces it** | 10.1 replaced "zero radius, never nested" with a two-level scale and one radius token. `tokens.test.ts` can assert a token's value; it cannot assert that nobody wrote `rounded-lg` on a button or nested a card block inside a card block, and Tailwind emits an unknown utility as *nothing*, so both mistakes look plausible in a screenshot. That is exactly where the alpha modifiers sat before open question 7 was answered by measurement. A lint rule over `src/**/*.tsx` for `rounded-` outside the one token is the obvious fix and is not written | Nothing yet; a rule that depends on code review |
