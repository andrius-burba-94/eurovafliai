...
| D12 | Phase 2's rehearsal draft | **Waived** (2026-09-08). The DoD asked for "a full 13-round mock draft completed on phone + PC with one member on autodraft throughout; a mid-draft rollback executed and the draft finishes clean". Every *mechanism* it names is built and tested, and the ceremony is dropped rather than deferred again — it had been the last open item on a phase that has been code-complete for five slices, and a DoD nobody intends to perform is a DoD that quietly makes every phase after it provisional. What replaces it is not nothing: a full 13-round two-member draft has been autodrafted end to end locally, `commitPick` is the one path a human pick and an autodraft share, rollback is covered by `computeRollback`'s engine tests plus two E2E specs, and 3.5 means a rollback now leaves a record you can read afterwards. **What is genuinely untested is what only people can test**: whether draft night *feels* right on eight phones in one room. That is Phase 3.7's own DoD ("a rehearsal draft night with 3+ friends, mixed devices") and it is the right place for it — one rehearsal, once, against the finished draft-day experience rather than against a half-built one. Phase 2 is **complete**. ✔ |
