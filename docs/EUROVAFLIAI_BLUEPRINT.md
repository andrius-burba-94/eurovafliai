...
- **3.6 Commissioner console.** ~~Pause/resume, rollback UI, toggle autodraft for absent members, adjust timer mid-draft, **manual pick entry** (commissioner mode / offline draft support).~~ **Cut — see D13.** Pause/resume and a working rollback shipped in 2.4, "Pick for them" *is* manual pick entry, and each remaining item already has a path. 3.6a (start over) and 3.6b (delete the league) shipped and stay.
